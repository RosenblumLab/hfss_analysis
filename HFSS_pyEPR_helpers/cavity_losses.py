...
from HFSS_pyEPR_helpers.qunatum_analysis import Simulation
...
def get_quality_factor(simulation, design_name=None, modes=[0], quality_factor_name=None, print_results=False):
    '''
    ...
    eprh = simulation.HFSS_analyze(design_name)
    df = eprh.get_freqs_bare_pd(eprh.variations[0])

    for mode in modes:
        if f'mode {mode}' not in simulation.quality_factors:
            simulation.quality_factors[f'mode {mode}'] = {}
        simulation.quality_factors[f'mode {mode}'][quality_factor_name] = df['Quality Factor'][mode]
...
def calculate_seam_loss(simulation: Simulation, seam_line: str, volume: str, g_seam: float=1e6):
    ...
    losses_data = calculate_losses_data(simulation)
    
    for mode_name, mode in simulation.format_dict.items():
        if mode_name not in simulation.quality_factors:
            simulation.quality_factors[mode_name] = {}
        print(f'\n{mode_name} results:')
        losses_data.eprh_seam.set_mode(mode)

        omega = 2 * np.pi * losses_data.F_seam[mode] * 1e9

        calcobject = CalcObject([], losses_data.eprh_seam.setup)
        ...
        simulation.quality_factors[mode_name]['seam_loss'] = Q_seam
    return

def calculate_GF_factors(simulation: Simulation, epsilon_r, t_h, volume='Vacuum'):
    '''
    ...
    Typical values $\epsilon_r = 33$ and $t_h = 5 \ nm$ dielectric thickness were used in https://journals.aps.org/prl/pdf/10.1103/PhysRevLett.119.264801,
    yielding F~3e-9, or even 1.2e-8 in https://link.aps.org/doi/10.1103/PhysRevApplied.13.034032.
    . The nominator integral of the filling factor is divided by the relative premittivity since the electric field ($\vec{E}$)
    ...
    '''
    print(simulation.eigenmodes.empty)
    if simulation.eigenmodes.empty:
        raise ValueError("The simulation must be solved before calculating Q-factors.")
    eprh_GF_factors = epr.DistributedAnalysis(simulation.project.pinfo)
    ...
    for mode_name, mode in simulation.format_dict.items():
        ...
        simulation.quality_factors[mode_name]['geometry_factor'] = G
        ...
        simulation.quality_factors[mode_name]['filling_factor'] = F

    return

def calculate_surface_losses(simulation, design_name, epsilon_r, t, volume, metal_surfaces, substrate,
    ...
    eprh_surface = simulation.HFSS_analyze(design_name)
    ...
    for mode in modes:
        ...
        simulation.quality_factors[f'mode {mode}']['surface_losses'] = Q_surface_total

if __name__ == "__main__":
    from HFSS_pyEPR_helpers.qunatum_analysis import Project, Sweep, Variable, Simulation

    project = Project(
        project_directory=r'E:\CavityLocalSimulations\Losses_20220105',
        project_name='CavityLosses_2',  # File name
        design_name='GF_Factors'
    )
    ...
    # setup.passes = 4  # Maximum number of passes
    # results = simulation.make_classic()
    # calculate_GF_factors(simulation, epsilon_r=33, t_h=5e-9, volume='Vacuum')

    simulation.project.design_name = 'SeamLoss'
    simulation.project.open()
    simulation.format_dict = {
        'Transmon': 0,
        'Cavity': 1,
        'Readout': 6
    }
    # losses_data = calculate_losses_data(simulation)
    results_seam = simulation.make_classic()
    calculate_seam_loss(simulation, g_seam=1e6, seam_line='SeamLine', volume='Vacuum')

    print(simulation.quality_factors)
